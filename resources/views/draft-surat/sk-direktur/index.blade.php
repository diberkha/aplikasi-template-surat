...
@section('title', 'Draft Surat Keputusan Direktur')

@section('content')
    <div class="space-y-6" x-data="draftSk()"
        @update-sk-draft.window="allData = allData.map(item => item.id_surat === $event.detail.id_surat ? $event.detail : item)">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Draft Surat Keputusan Direktur</h1>
                <p class="text-gray-600 dark:text-gray-400 mt-1">
                    Daftar Surat Keputusan Direktur yang masih berstatus draft dan belum diarsipkan
                </p>
            </div>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-900 dark:text-white">
                                    <div class="truncate max-w-xs lg:max-w-md"
                                        :title="item.sk_direktur ? item.sk_direktur.tentang : item.nama_surat"
                                        x-text="item.sk_direktur ? item.sk_direktur.tentang : item.nama_surat"></div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900 dark:text-white" x-text="item.nomor_surat || '-'">
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex items-center justify-center space-x-2">
                                        ...
                                        <button
                                            @click="arsipkanDraft(item.id_surat, item.sk_direktur ? item.sk_direktur.tentang : item.nama_surat)"
                                        ...
                                        <button
                                            @click="deleteDraft(item.id_surat, item.sk_direktur ? item.sk_direktur.tentang : item.nama_surat)"
    ...
    @include('draft-surat.sk-direktur.partials.modal-edit')
    @include('draft-surat.sk-direktur.partials.modal-archive')
    @include('draft-surat.sk-direktur.partials.modal-delete')
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('draftSk', () => ({
                ...
                get filteredData() {
                    ...
                    if (this.search) {
                        ...
                        data = data.filter(item => {
                            const tentang = item.sk_direktur ? item.sk_direktur.tentang : item.nama_surat;
                            const nomor = item.nomor_surat;
                            return tentang.toLowerCase().includes(s) || nomor.toLowerCase().includes(s);
                        });
                    }
                    ...
                    if (this.sortOption === 'a-z') {
                        data.sort((a, b) => {
                            const valA = a.sk_direktur ? a.sk_direktur.tentang : a.nama_surat;
                            const valB = b.sk_direktur ? b.sk_direktur.tentang : b.nama_surat;
                ...
                arsipkanDraft(id, title) {
                    document.getElementById('arsipkan-nama-sk').textContent = title;
                    const btnConfirm = document.getElementById('btnConfirmArsipkanSk');

                    const newBtn = btnConfirm.cloneNode(true);
                    btnConfirm.parentNode.replaceChild(newBtn, btnConfirm);

                    newBtn.addEventListener('click', () => {
                        fetch("{{ route('sk-direktur.archive', ':id') }}".replace(':id', id), {
                            method: 'POST',
                            headers: {
                    ...
                    openModal('modalArsipkanSk');
                },

                deleteDraft(id, title) {
                    document.getElementById('delete-nama-sk').textContent = title;
                    const btnConfirm = document.getElementById('btnConfirmDeleteSk');

                    const newBtn = btnConfirm.cloneNode(true);
                    btnConfirm.parentNode.replaceChild(newBtn, btnConfirm);

                    newBtn.addEventListener('click', () => {
                        fetch(`{{ route('arsip-surat.destroy', '') }}${id}`, {
                            method: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                'Content-Type': 'application/json'
