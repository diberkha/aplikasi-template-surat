...
@section('title', 'Data Cuti Bersama')

@section('content')
    <div x-data="cutiBersamaTable()" class="space-y-6">

        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Data Cuti Bersama</h1>
                <p class="text-gray-600 dark:text-gray-400 mt-1">Kelola informasi data cuti bersama</p>
            </div>
                ...
                <div class="flex items-center gap-2 w-full sm:w-auto">
                    <div class="relative flex-1 sm:flex-initial group">
                        ...
                        <input type="text" x-model="search" placeholder="Cari..."
                            class="pl-9 pr-10 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 dark:bg-gray-700 dark:text-white w-full sm:w-48 lg:w-64 text-sm transition-all outline-none">
        ...
        <div
            ...
            <div
                ...
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Daftar Cuti Bersama</h3>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                            ...
                            <th
                                class="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                                Diperhitungkan sebagai Cuti Tahunan</th>
                            <th
                                class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                                Actions</th>
                        </tr>
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        <template x-for="(item, index) in paginatedData()" :key="item.id">
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                ...
                                <td class="px-6 py-4 text-gray-700 dark:text-gray-300" x-text="item.jenis_cuti_bersama">
                                </td>
                                <td class="px-6 py-4 text-gray-700 dark:text-gray-300" x-text="item.tahun"></td>
                                <td class="px-6 py-4 text-center text-gray-700 dark:text-gray-300">
                                    <span x-text="item.jumlah_hari"></span> Hari
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <template x-if="item.is_perhitungan_cuti_tahunan">
                                        <span
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <button @click="openDeleteModal(item.id, item.jenis_cuti_bersama)"
            ...
            @include('cuti-bersama.partials.modal-create')
            @include('cuti-bersama.partials.modal-edit')
            @include('cuti-bersama.partials.modal-delete')

        </div>

        <script>
            function cutiBersamaTable() {
                return {
                    search: '',
                    sortOption: null,
                    data: @json($cutisBersama),
                    itemsPerPage: 10,
                    currentPage: 1,
                    ...
                    filteredSortedData() {
                        let filtered = this.data.filter(item => {
                            ...
                            return item.jenis_cuti_bersama.toLowerCase().includes(searchLower) ||
                                item.tahun.toString().includes(searchLower);
                        });

                        switch (this.sortOption) {
                            case 'a-z':
                                filtered.sort((a, b) => a.jenis_cuti_bersama.localeCompare(b.jenis_cuti_bersama));
                                break;
                            case 'z-a':
                                filtered.sort((a, b) => b.jenis_cuti_bersama.localeCompare(a.jenis_cuti_bersama));
                                break;
                            case 'latest':
                    ...
                    openCreateModal() {
                        document.getElementById('formCreateCutiBersama').reset();
                        openModal('modalCreateCutiBersama');
                    },

                    openEditModal(id) {
                        openModal('modalEditCutiBersama');
                        fetch(`{{ route('cuti-bersama.index') }}/${id}/detail`)
                            .then(r => r.json())
                            .then(data => {
                                document.getElementById('edit_id_cuti').value = data.id;
                                document.getElementById('edit_nama_cuti').value = data.jenis_cuti_bersama;
                                document.getElementById('edit_tahun').value = data.tahun;
                                document.getElementById('edit_jumlah_hari').value = data.jumlah_hari;
                                document.getElementById('edit_potong_jatah_tahunan').checked = data.is_perhitungan_cuti_tahunan;
                                document.getElementById('edit_catatan').value = data.catatan || '';
                                document.getElementById('formEditCutiBersama').action = `{{ route('cuti-bersama.index') }}/${id}`;

                                if (typeof FormDirtyMonitor !== 'undefined') {
                                    new FormDirtyMonitor('formEditCutiBersama', 'btnSubmitEditCutiBersama');
                                }
                            });
                    },

                    openDeleteModal(id, nama) {
                        openModal('modalDeleteCutiBersama');
                        document.getElementById('delete-nama-cuti').textContent = nama;
                        document.getElementById('formDeleteCutiBersama').action = `{{ route('cuti-bersama.index') }}/${id}`;
                    }
                }
            }
        </script>
