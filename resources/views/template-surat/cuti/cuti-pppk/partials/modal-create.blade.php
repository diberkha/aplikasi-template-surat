        ...
        <form action="{{ route('template-surat.cuti.store') }}" method="POST" id="cutiFormPPPK"
            ...
            <div class="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
                    ...
                    <div class="p-4">
                        <div>
                            ...
                            <div class="relative" @click.outside="openJenisCuti = false">
                                <input type="hidden" name="form[jenis_cuti]" id="jenis_cuti_pppk" :value="jenisCuti" required>
                ...
                <div class="border border-gray-300 dark:border-gray-600 rounded-lg overflow-hidden">
                    ...
                    <div class="p-4">
                        <input type="hidden" name="form[rentang_cuti_json]" id="rentang_cuti_json_pppk">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block mb-2 text-gray-700 dark:text-gray-300">Selama (hari) <span
                                        class="text-red-500">*</span></label>
                                <input type="number" name="form[lama_cuti]" id="lama_cuti_pppk" required min="1" readonly
                                    class="w-full px-4 py-3 bg-gray-100 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-600 dark:text-gray-300 cursor-not-allowed">
                            </div>
                            <div>
                                <label class="block mb-2 text-gray-700 dark:text-gray-300">Tanggal Cuti <span
                                        class="text-red-500">*</span></label>
                                <div class="relative">
                                    <input type="text" id="tanggal_cuti_multi_pppk" placeholder="Pilih tanggal cuti"
                                        class="w-full px-4 py-3 pr-10 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white">
                                    <button type="button" id="clear_tanggal_cuti_pppk"
                                        class="hidden absolute inset-y-0 right-0 pr-3 flex items-center text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors"
                        ...
                        <input type="hidden" name="form[mulai]" id="mulai_cuti_pppk" required>
                        <input type="hidden" name="form[sampai]" id="sampai_cuti_pppk" required>
                    </div>
                </div>
...
<script>
    (function () {
        ...
        searchInput.addEventListener('input', function (e) {
            ...
            dobounceTimer = setTimeout(() => {
                ...
                ).filter(p => p.jenis_pegawai === 'PPPK').slice(0, 10);
        ...
        function selectPegawai(id) {
            ...
            if (data) {
                ...
                if (data.masa_kerja) {
                    ...
                    document.getElementById('masa_kerja_tahun_pppk').value = years;
                    document.getElementById('masa_kerja_bulan_pppk').value = months;
                } else {
                    document.getElementById('masa_kerja_tahun_pppk').value = 0;
                    document.getElementById('masa_kerja_bulan_pppk').value = 0;
                }
        ...
        const jenisCutiSelect = document.getElementById('jenis_cuti_pppk');
        ...
        if (modal) {
            modal.addEventListener('modal-closed', function () {
                ...
                if (typeof initRentangCutiPPPK === 'function') {
                    initRentangCutiPPPK();
                }
            });
        }
    ...
    function attachFlatpickrActionsPPPK(instance) {
    ...
    function initRentangCutiPPPK() {
        const tanggalInput = document.getElementById('tanggal_cuti_multi_pppk');
        const clearBtn = document.getElementById('clear_tanggal_cuti_pppk');
    ...
    function syncRentangPPPK(selectedDates = null) {
        const lamaInput = document.getElementById('lama_cuti_pppk');
        const mulaiInput = document.getElementById('mulai_cuti_pppk');
        const sampaiInput = document.getElementById('sampai_cuti_pppk');
        const jsonInput = document.getElementById('rentang_cuti_json_pppk');
        const tanggalInput = document.getElementById('tanggal_cuti_multi_pppk');
        const clearBtn = document.getElementById('clear_tanggal_cuti_pppk');
        ...
        if (tanggalInput._flatpickr && tanggalInput._flatpickr.altInput) {
            const compactLabel = formatCompactRangeLabelPPPK(ranges);
    ...
    function formatCompactRangeLabelPPPK(ranges) {
    ...
    document.addEventListener('DOMContentLoaded', initRentangCutiPPPK);
    ...
    function submitCutiFormPPPK(e) {
        ...
        const rentangJson = document.getElementById('rentang_cuti_json_pppk');
