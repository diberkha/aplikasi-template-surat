<div id="modalDeleteCutiPPPK" x-data="{ isOpen: false }" x-show="isOpen" x-cloak
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full relative">
        ...
        <div class="p-6">
            ...
            <div class="mt-4 bg-green-50 dark:bg-green-900/20 p-4 rounded-lg border border-green-200 dark:border-green-800">
                <p class="text-sm mb-2">
                    ...
                    <span id="delete-cuti-pppk-template-name" class="text-gray-800 dark:text-gray-200">-</span>
                </p>
            </div>
        </div>
        <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex justify-end space-x-3">
            ...
            <form id="formDeleteTemplateCutiPPPK" method="POST">
                @csrf
                @method('DELETE')
...
<script>
    function openDeleteModalPPPK(id, name) {
        const modalEl = document.getElementById('modalDeleteCutiPPPK');
        if (!modalEl) return;
        
        const nameEl = modalEl.querySelector('#delete-cuti-pppk-template-name');
        if (nameEl) nameEl.textContent = name;
        
        const form = modalEl.querySelector('#formDeleteTemplateCutiPPPK');
        if (form) {
