<div id="modalDeleteCutiNonASN" x-data="{ isOpen: false }" x-show="isOpen" x-cloak
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full relative">
        ...
        <div class="p-6">
            ...
            <div class="mt-4 bg-red-50 dark:bg-red-900/20 p-4 rounded-lg border border-red-200 dark:border-red-800">
                <p class="text-sm mb-2">
                    ...
                    <span id="delete-cuti-nonasn-template-name" class="text-gray-800 dark:text-gray-200">-</span>
                </p>
            </div>
        </div>
        <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex justify-end space-x-3">
            ...
            <form id="formDeleteTemplateCutiNonASN" method="POST">
                @csrf
                @method('DELETE')
...
<script>
    function openDeleteModalNonASN(id, name) {
        const modalEl = document.getElementById('modalDeleteCutiNonASN');
        if (!modalEl) return;

        const nameEl = modalEl.querySelector('#delete-cuti-nonasn-template-name');
        if (nameEl) nameEl.textContent = name;

        const form = modalEl.querySelector('#formDeleteTemplateCutiNonASN');
        if (form) {
